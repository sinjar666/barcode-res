...
void ReadEAN13::getLimits() {
    ...
    binary=QImage(right-left+1,100,QImage::Format_Mono);
    for(uint i=0;i<=right-left;i++)    {
        for(uint j=0;j<100;j++)   {
            if(qRed(tmp.pixel(left+i,j))==0)
                binary.setPixel(i,j,0);
            else
                binary.setPixel(i,j,1);
        }
    ...
    tmp=binary.scaled(950,100,Qt::IgnoreAspectRatio);
    QFile fil2("out3.bmp");
...
void ReadEAN13::prepare()   {
    tmp=QImage(1024,100,QImage::Format_Mono);
    for(uint i=0;i<=1023;i++)    {
        for(uint j=0;j<100;j++)   {
            if(qRed(binary.pixel(i,r+j))==0)
                tmp.setPixel(i,j,0);
            else
                tmp.setPixel(i,j,1);
        }
