...
void StillCam::paintEvent(QPaintEvent *event)   {
    ...
    if(!isActiveWindow())   {
        ...
        painter.drawText(*bound,Qt::AlignCenter,QString("Camera Standby"),bound);
        painter.restore();
    }
}
...
StillCam::StillCam(QWidget *parent) :
    QWidget(parent)
{
    ...
    Path="/home/user/MyDocs/sample.jpg";
    pencolor=Qt::white;
    ...
    setupCameraBin();
    connect(this,SIGNAL(pictDone()),this,SLOT(doBarcodeRead()));
    if(isShutterOpen())
        g_digicam_manager_play_bin(manager, winId(), NULL);
    else    {
...
void StillCam::doBarcodeRead()  {
    reader=new ReadEAN13(Path);
    connect(reader,SIGNAL(finished()),this,SLOT(checkBarcode()));
    reader->start();
}

void StillCam::checkBarcode()   {
    if(reader->getResult()=="") {
        qDebug() << "Try Again...";
        delete(reader);
        g_digicam_manager_play_bin(manager,winId(),NULL);
        ...
        qDebug() << "Result: " +reader->getResult();
        delete(reader);
        close();
    }
}
